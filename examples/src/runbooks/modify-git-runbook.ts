...
const main = async () => {
    ...
    runbook.Description = "This is a modified runbook";
    const modifiedRunbook = await runbookRepo.modifyWithGitRef(runbook, "main");
  
    if (modifiedRunbook !== null && modifiedRunbook !== undefined) {
        console.log(`Runbook modified: "${modifiedRunbook.Name}" (${modifiedRunbook.Id})`);
    } else {
        console.error(`Runbook, "${runbookId}" not modified`);
        return;
    }
};
