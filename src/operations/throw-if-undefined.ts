...
export async function throwIfUndefined<T extends IIdName>(
    ...
    if (!new RegExp(`^${escapeRegExp(resourceTypeIdPrefix)}-/d+$`).test(nameOrId)) {
    ...
    if (resourceById === undefined && resourceByName === undefined)
        throw CouldNotFindError.createResource(resourceTypeDisplayName, nameOrId, enclosingContextDescription);
