...
export class ReleasePlanBuilder {
  ...
  constructor(
    ...
    readonly versionResolver: IPackageVersionResolver,
    readonly channelVersionRuleTester: IChannelVersionRuleTester
  ) {}

  private static async loadFeedsForSteps(
    repository: OctopusSpaceRepository,
    project: ProjectResource,
    steps: ReleasePlanItem[]
  ) {
    // PackageFeedId can be an id or a name
    const allRelevantFeedIdOrName = steps
      .map(step => step.packageFeedId)
    ...
    return new Map(
      project.IsVersionControlled
        ? allRelevantFeeds.Items.map(p => [p.Name, p])
        : allRelevantFeeds.Items.map(p => [p.Id, p])
    );
  }

  private buildChannelVersionFilters(
    stepName: string,
  ...
  async build(
    ...
    return !gitReference
      ? await this.buildReleaseFromDatabase(
          repository,
          project,
          channel,
          versionPreReleaseTag
        )
      : await this.buildReleaseFromVersionControl(
          repository,
          project,
          channel as ChannelResource,
          versionPreReleaseTag,
          gitReference,
          gitCommit
        );
  ...
  async buildReleaseFromDatabase(
    ...
    if (project.IsVersionControlled)
      throw new Error(
        GitReferenceMissingForVersionControlledProjectErrorMessage
      );
    ...
    if (releaseTemplate === undefined)
      throw new CouldNotFindError(
        channel
          ...
          : `a release template for project ${project.Name}`
      );
  ...
  async buildReleaseFromVersionControl(
    ...
    const gitObjectName = !gitCommit
      ? `reference ${gitReference}`
      : `commit ${gitCommit}`;

    if (!project.IsVersionControlled)
      throw new Error(
        ReleasePlanBuilder.gitReferenceSuppliedForDatabaseProjectErrorMessage(
          gitObjectName
        )
      );
    ...
    if (deploymentProcess === undefined)
      throw new CouldNotFindError(
        `a deployment process for project ${project.Name} and git ${gitObjectName}`
      );
    ...
    if (releaseTemplate === undefined)
      throw new CouldNotFindError(
        `a release template for project ${project.Name}, channel ${channel.Name} and git ${gitObjectName}`
      );
  ...
  private async buildInternal(
    ...
    if (plan.unresolvedSteps.length > 0) {
      repository.client.debug(
        'The package version for some steps was not specified. Going to try and resolve those automatically...'
      );

      const allRelevantFeeds = await ReleasePlanBuilder.loadFeedsForSteps(
        repository,
        project,
        plan.unresolvedSteps
      );
      ...
      for (const unresolved of plan.unresolvedSteps) {
        ...
        if (versionPreReleaseTag)
          repository.client.debug(
            `Finding latest package with pre-release '${versionPreReleaseTag}' for step: ${unresolved.actionName}`
          );
        ...
        if (!allRelevantFeeds.has(unresolved.packageFeedId as string)) {
          throw new Error(
            `Could not find a feed with ID ${unresolved.packageFeedId}, which is used by step: ${unresolved.actionName}`
          );
        }
        const feed = allRelevantFeeds.get(
          unresolved.packageFeedId as string
        ) as FeedResource;
        const filters = this.buildChannelVersionFilters(
          unresolved.actionName,
          unresolved.packageReferenceName as string,
          channel
        );
        ...
        const packages = await this.client.get<PackageResource[]>(
          feed.Links.SearchTemplate,
          filters
        );
        ...
        if (packages.length === 0) {
          ...
          repository.client.debug(
            `Selected '${latestPackage.PackageId}' version '${latestPackage.Version}' for '${unresolved.actionName}'`
          );
