...
export class PackageVersionResolver implements IPackageVersionResolver {
  static readonly SupportedZipFilePatterns = [
    '*.zip',
    '*.tgz',
    '*.tar.gz',
    '*.tar.Z',
  ...
  async addFolder(folderPath: string) {
    ...
    this.client.debug(`Using package versions from folder: ${folderPath}`);
    const files = await retrievePackages(
      `*(${PackageVersionResolver.SupportedZipFilePatterns.map(
        v => `**/${v}`
      ).join('|')})`
    );
  ...
  add3(stepNameOrPackageIdAndVersion: string) {
    ...
    if (split.length < 2)
      throw new Error(
        `The package argument '${stepNameOrPackageIdAndVersion}' does not use expected format of : {Step Name}:{Version}`
      );
  ...
  add2(stepNameOrPackageId: string, packageVersion: string) {
  ...
  tryParseIdAndVersion(filename: string): PackageIdentity | undefined {
    ...
    const match = new RegExp(
      `^${packageIdPattern.source}.${semanticVersionPattern.source}$`
    ).exec(idAndVersion);
  ...
  resolveVersion(
    ...
    return (
      ...
      identifiers
        .flatMap(id => [
          packageKey(WildCard, packageReferenceName ?? ''),
          packageKey(id, WildCard)
        ])
        .map(key => this.stepNameToVersion.get(key) ?? undefined)
        .find(version => version !== undefined) ??
      this.defaultVersion
